...
function OnboardingContent() {
  ...
  const modeText = {
    title: authMode ? "Welcome Back 👋" : "Create Your Account 🚀",
    subtitle: authMode
      ? "Log in to continue to your dashboard."
      : "Sign up to start simplifying your taxes with Paycort.",
    button: authMode ? "Log In" : "Sign Up",
    toggle: authMode ? "Don't have an account?" : "Already have an account?",
    toggleLink: authMode ? "Sign Up" : "Log In",
    modalTitle: authMode ? "Welcome Back!" : "Account Created!",
    modalMessage: authMode
  ...
  return (
    <>
          ...
          <div className="flex justify-center gap-6 mb-8 border-b border-gray-200 w-full">
            {["login", "signup"].map((mode) => (
              <button
                ...
                className={`cursor-pointer pb-3 font-medium text-sm transition-all duration-300 ${
                  (mode === "login" && authMode) ||
                  (mode === "signup" && !authMode)
      ...
      <Modal
        ...
        title={modeText.modalTitle}
        message={modeText.modalMessage}
      />
    </>
  );
}
